...
int ls_inspect_fields(const SceneDesc *d, int sel_light, int sel_prim,
    ...
    if (sel_light >= 0 && sel_light < d->nlights) {
        ...
        /* Flux. Lumens is the number on the box, so it leads at every tier;
         * watts appear alongside it only for the scientific reader. */
        if (l->kind != LS_LIGHT_DIRECTIONAL) {
            push(out, max, &n, val(FLD_L_LM, "FLUX", "LM",
                                   light_lumens(l), 0.0, 1e7));
            if (sci) {
                push(out, max, &n, val(FLD_L_W, "RADIANT FLUX", "W",
                                       l->phi_e, 0.0, 1e5));
            ...
            push(out, max, &n, val(FLD_L_W, "IRRADIANCE", "W/M2",
                                   l->e_perp, 0.0, 1e5));
        }
...
static void respec(Light *l) {
    switch (l->spd_kind) {
        case LS_SPD_BLACKBODY: l->s_hat = ls_spectrum_blackbody(l->spd_a); break;
        case LS_SPD_DAYLIGHT:  l->s_hat = ls_spectrum_daylight(l->spd_a);  break;
        case LS_SPD_LED:       l->s_hat = ls_spectrum_gaussian(l->spd_a, l->spd_b, 1.0); break;
        case LS_SPD_FLAT:
        default:               l->s_hat = ls_spectrum_const(1.0); break;
    }
